...
class ChartInfo(BaseModel):
    ...
    @property
    def ra(self) -> int:
        return computeRa(self.ds, self.achievements)
...
class DrawBest(Draw):
    ...
    def __init__(self, UserInfo: UserInfo, qqId: Optional[Union[int, str]] = None) -> None:
        ...
        self.Rating = sum([_.ra for _ in self.sdBest]) + sum([_.ra for _ in self.dxBest]) + 2100
    ...
    async def draw(self) -> Image.Image:
        ...
        plate_folder = Path(maimaidir) / 'nameplates'
        plate_files = list(plate_folder.glob('*.png'))
        icon_folder = Path(maimaidir) / 'icons'
        icon_files = list(icon_folder.glob('*.png'))

        self._im.alpha_composite(logo, (5, 130))
        if self.plate:
            plate = Image.open(platedir / f'{self.plate}.png').resize((1420, 230))
        else:
            plate = Image.open(random.choice(plate_files)).resize((1420, 230))
        self._im.alpha_composite(plate, (390, 100))
        icon = Image.open(random.choice(icon_files)).resize((214, 214))
        ...
        self._tb.draw(847, 295, 28, f'{sdrating} + {dxrating} + 2100 = {self.Rating}', (0, 0, 0, 255), 'mm', 3, (255, 255, 255, 255))
...
async def generate_best_40(qqid: Optional[int] = None, username: Optional[str] = None) -> str:
    try:
        if username:
            qqid = None
