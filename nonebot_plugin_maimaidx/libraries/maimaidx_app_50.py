...
class DrawBest(Draw):
    ...
    def __init__(self, UserInfo: UserInfo, qqId: Optional[Union[int, str]] = None) -> None:
        ...
        # 选出app
        self.sdBest = [i for i in self.sdBest if i.fc in ['app']]
        self.dxBest = [i for i in self.dxBest if i.fc in ['app']]
    ...
    async def draw(self) -> Image.Image:
        ...
        self._tb.draw(847, 295, 28, f'(AP+50) B35: {sdrating} + B15: {dxrating}', (0, 0, 0, 255), 'mm', 3, (255, 255, 255, 255))
...
async def generate_app_50(qqid: Optional[int] = None, username: Optional[str] = None) -> str:
    try:
        if username:
            qqid = None
